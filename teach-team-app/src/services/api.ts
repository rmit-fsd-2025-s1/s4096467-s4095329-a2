...
export const userApi = {
  ...
  searchData: async (sort: string, filter: string, search: string, availability: string, type: string) =>{
    const response = await api.get(`/classes/search/${sort}/${filter}/${search.length == 0? "@undef": search}/${availability === "None" ? availability : availability.toLowerCase()}/${type.toLowerCase()}`);
    return response.data;
  },
